...
# ==============================================================================
# OpenClawDeck - One-Click Launcher
# ==============================================================================
...
# 获取最新版本号
REPO="OpenClawDeck/OpenClawDeck"
...
echo -e "${CYAN}:: OpenClawDeck Launcher - ${LATEST_VERSION} ::${NC}"
echo ""
...
if [ "$(id -u)" = "0" ]; then
    ...
    if id "openclaw" &>/dev/null; then
        ...
        echo -e "  ${GREEN}curl -fsSL https://raw.githubusercontent.com/OpenClawDeck/OpenClawDeck/main/install.sh | bash${NC}"
        echo ""
        exit 0
    fi
...
# Asset name convention: openclawdeck-{os}-{arch} (e.g., openclawdeck-linux-amd64)
# If on macOS, the binary might be named openclawdeck-darwin-amd64 or similar.
# Adjusting based on standard Go build naming.
BINARY_NAME="openclawdeck"
ASSET_PATTERN="openclawdeck-${OS}-${ARCH}"

# Fallback for Windows (if run in git bash/wsl)
if [[ "$OS" == *"mingw"* || "$OS" == *"cygwin"* ]]; then
    OS="windows"
    ASSET_PATTERN="openclawdeck-windows-${ARCH}.exe"
    BINARY_NAME="openclawdeck.exe"
fi
